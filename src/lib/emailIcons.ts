// Email-safe SVG icons as base64 data URIs for use in HTML email templates
// These icons are designed to work across all major email clients

// Truck icon - Green (for shipment received)
export const ICON_TRUCK_GREEN = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/><path d="M15 18h2.586a1 1 0 0 0 .707-.293l3.414-3.414a1 1 0 0 0 .293-.707V13a1 1 0 0 0-1-1h-4v6z"/><circle cx="17" cy="18" r="2"/><circle cx="7" cy="18" r="2"/></svg>`)}`;

// Truck icon - Primary/Orange (for status changed)
export const ICON_TRUCK_PRIMARY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/><path d="M15 18h2.586a1 1 0 0 0 .707-.293l3.414-3.414a1 1 0 0 0 .293-.707V13a1 1 0 0 0-1-1h-4v6z"/><circle cx="17" cy="18" r="2"/><circle cx="7" cy="18" r="2"/></svg>`)}`;

// Truck icon - Orange (for releases/outbound)
export const ICON_TRUCK_ORANGE = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#FD5A2A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 18V6a2 2 0 0 0-2-2H4a2 2 0 0 0-2 2v11a1 1 0 0 0 1 1h2"/><path d="M15 18h2.586a1 1 0 0 0 .707-.293l3.414-3.414a1 1 0 0 0 .293-.707V13a1 1 0 0 0-1-1h-4v6z"/><circle cx="17" cy="18" r="2"/><circle cx="7" cy="18" r="2"/></svg>`)}`;

// Package icon - Green (for items received)
export const ICON_PACKAGE_GREEN = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16.5 9.4 7.5 4.21"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.29 7 12 12 20.71 7"/><line x1="12" y1="22" x2="12" y2="12"/></svg>`)}`;

// Warning triangle - Red (for damage)
export const ICON_WARNING_RED = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="#fef2f2" stroke="#dc2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m21.73 18-8-14a2 2 0 0 0-3.48 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3Z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>`)}`;

// Red X Circle (for unable to complete)
export const ICON_X_CIRCLE_RED = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="#fef2f2" stroke="#dc2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="15" y1="9" x2="9" y2="15"/><line x1="9" y1="9" x2="15" y2="15"/></svg>`)}`;

// Clipboard icon - Primary/Orange (for tasks)
export const ICON_CLIPBOARD_PRIMARY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><path d="M9 14l2 2 4-4"/></svg>`)}`;

// Clock alert - Red (for overdue)
export const ICON_CLOCK_RED = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="#fef2f2" stroke="#dc2626" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>`)}`;

// Check circle - Green (for completed/success)
export const ICON_CHECK_GREEN = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="#f0fdf4" stroke="#16a34a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>`)}`;

// Location pin - Primary/Orange (for location changes)
export const ICON_LOCATION_PRIMARY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0Z"/><circle cx="12" cy="10" r="3"/></svg>`)}`;

// Document/Invoice - Orange (for invoices)
export const ICON_DOCUMENT_ORANGE = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#FD5A2A" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>`)}`;

// Dollar circle - Green (for payments)
export const ICON_DOLLAR_GREEN = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="#f0fdf4" stroke="#16a34a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><path d="M16 8h-6a2 2 0 1 0 0 4h4a2 2 0 1 1 0 4H8"/><path d="M12 18V6"/></svg>`)}`;

// Wrench - Amber (for repairs)
export const ICON_WRENCH_AMBER = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#f59e0b" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>`)}`;

// User pickup - Green (for will call)
export const ICON_USER_GREEN = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#16a34a" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>`)}`;

// Magnifying glass - Primary/Orange (for inspection)
export const ICON_MAGNIFY_PRIMARY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>`)}`;

// Forklift - Gray (for warehouse ops)
export const ICON_FORKLIFT_GRAY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 12H5a2 2 0 0 0-2 2v5"/><circle cx="13" cy="19" r="2"/><circle cx="5" cy="19" r="2"/><path d="M8 19h3m5-17v17h6M6 12V7c0-1.1.9-2 2-2h3l5 5"/></svg>`)}`;

// Boxes/Package stack - Gray (for custom/generic)
export const ICON_BOXES_GRAY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16.5 9.4 7.5 4.21"/><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><polyline points="3.29 7 12 12 20.71 7"/><line x1="12" y1="22" x2="12" y2="12"/></svg>`)}`;

// Hand truck/Dolly - Primary/Orange (for receiving)
export const ICON_DOLLY_PRIMARY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 18L6 6.5C6 5.11929 7.11929 4 8.5 4H8.5C9.88071 4 11 5.11929 11 6.5V8"/><path d="M6 18H4"/><path d="M6 18C6 19.6569 7.34315 21 9 21C10.6569 21 12 19.6569 12 18"/><rect x="11" y="6" width="8" height="10" rx="1"/></svg>`)}`;

// User Plus - Primary/Orange (for employee invite)
export const ICON_USER_PLUS_PRIMARY = `data:image/svg+xml;base64,${btoa(`<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="#ea580c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><line x1="19" y1="8" x2="19" y2="14"/><line x1="22" y1="11" x2="16" y2="11"/></svg>`)}`;

// Export icon map for easy access by trigger event
export const EMAIL_ICONS = {
  truck_green: ICON_TRUCK_GREEN,
  truck_primary: ICON_TRUCK_PRIMARY,
  truck_orange: ICON_TRUCK_ORANGE,
  package_green: ICON_PACKAGE_GREEN,
  warning_red: ICON_WARNING_RED,
  x_circle_red: ICON_X_CIRCLE_RED,
  clipboard_primary: ICON_CLIPBOARD_PRIMARY,
  clock_red: ICON_CLOCK_RED,
  check_green: ICON_CHECK_GREEN,
  location_primary: ICON_LOCATION_PRIMARY,
  document_orange: ICON_DOCUMENT_ORANGE,
  dollar_green: ICON_DOLLAR_GREEN,
  wrench_amber: ICON_WRENCH_AMBER,
  user_green: ICON_USER_GREEN,
  magnify_primary: ICON_MAGNIFY_PRIMARY,
  forklift_gray: ICON_FORKLIFT_GRAY,
  boxes_gray: ICON_BOXES_GRAY,
  dolly_primary: ICON_DOLLY_PRIMARY,
  user_plus_primary: ICON_USER_PLUS_PRIMARY,
} as const;

export type EmailIconKey = keyof typeof EMAIL_ICONS;

// Email template configuration for each trigger event
// Using fun, friendly, but professional messaging style
export interface EmailTemplateConfig {
  icon: EmailIconKey;
  accentColor: string;
  buttonColor: string;
  headerLine1: string;
  headerLine2: string;
  description: string;
  ctaText: string;
  ctaLink: string;
  includeItemsTable: boolean;
  isUrgent?: boolean;
  smsTemplate: string;
}

// Template configurations for all 17 trigger events - Fun, Friendly, Professional!
export const EMAIL_TEMPLATE_CONFIGS: Record<string, EmailTemplateConfig> = {
  'shipment.received': {
    icon: 'truck_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Your shipment',
    headerLine2: 'has arrived!',
    description: 'Great news, [[account_contact_name]]! Your shipment [[shipment_number]] just arrived at our facility. We\'ve checked in [[items_count]] items and they\'re safe and sound.',
    ctaText: 'View Shipment Details',
    ctaLink: '[[shipment_link]]',
    includeItemsTable: true,
    smsTemplate: '📦 [[tenant_name]]: Your shipment [[shipment_number]] has arrived! [[items_count]] items checked in safely. View details: [[portal_base_url]]',
  },
  'shipment.status_changed': {
    icon: 'truck_primary',
    accentColor: '#ea580c',
    buttonColor: '#111111',
    headerLine1: 'Shipment',
    headerLine2: 'Update',
    description: 'Heads up! Your shipment [[shipment_number]] status has changed to [[shipment_status]]. We\'re keeping you in the loop every step of the way.',
    ctaText: 'Track Your Shipment',
    ctaLink: '[[shipment_link]]',
    includeItemsTable: false,
    smsTemplate: '🚚 [[tenant_name]]: Shipment [[shipment_number]] is now [[shipment_status]]. Track it: [[portal_base_url]]',
  },
  'shipment.completed': {
    icon: 'check_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'All done!',
    headerLine2: 'Shipment complete.',
    description: 'Your shipment [[shipment_number]] is fully processed. All [[items_count]] items have been received and logged. Everything\'s ready to go!',
    ctaText: 'View Summary',
    ctaLink: '[[shipment_link]]',
    includeItemsTable: true,
    smsTemplate: '✅ [[tenant_name]]: Shipment [[shipment_number]] complete! All [[items_count]] items received. Details: [[portal_base_url]]',
  },
  'item.received': {
    icon: 'package_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Item safely',
    headerLine2: 'received!',
    description: 'Your item [[item_code]] has been received and stored at [[item_location]]. We\'ve got it covered!',
    ctaText: 'View Item',
    ctaLink: '[[item_photos_link]]',
    includeItemsTable: false,
    smsTemplate: '📦 [[tenant_name]]: Item [[item_code]] received and stored at [[item_location]]. View: [[portal_base_url]]',
  },
  'item.damaged': {
    icon: 'warning_red',
    accentColor: '#dc2626',
    buttonColor: '#dc2626',
    headerLine1: 'Damage',
    headerLine2: 'Detected',
    description: 'We noticed something that needs your attention. Item [[item_code]] has been flagged for potential damage. Please review the photos and let us know how you\'d like to proceed.',
    ctaText: 'Review Photos Now',
    ctaLink: '[[item_photos_link]]',
    includeItemsTable: false,
    isUrgent: true,
    smsTemplate: '🚨 URGENT [[tenant_name]]: Damage reported on item [[item_code]]. Please review photos immediately: [[portal_base_url]]',
  },
  'item.location_changed': {
    icon: 'location_primary',
    accentColor: '#ea580c',
    buttonColor: '#111111',
    headerLine1: 'Item on',
    headerLine2: 'the move!',
    description: 'Just letting you know: item [[item_code]] has been relocated to [[item_location]]. Same great care, new spot!',
    ctaText: 'View Item Location',
    ctaLink: '[[item_photos_link]]',
    includeItemsTable: false,
    smsTemplate: '📍 [[tenant_name]]: Item [[item_code]] moved to [[item_location]]. View: [[portal_base_url]]',
  },
  'item.flag_added': {
    icon: 'clipboard_primary',
    accentColor: '#ea580c',
    buttonColor: '#111111',
    headerLine1: 'Service',
    headerLine2: 'flagged!',
    description: 'Attention: [[user_name]] just added [[service_name]] to [[account_name]] account. Please review.',
    ctaText: 'View Details',
    ctaLink: '[[portal_base_url]]',
    includeItemsTable: false,
    smsTemplate: '🚩 [[tenant_name]]: [[user_name]] added [[service_name]] to [[account_name]]. Review: [[portal_base_url]]',
  },
  'billing_event.created': {
    icon: 'dollar_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Service',
    headerLine2: 'charged!',
    description: 'Attention: [[user_name]] just added [[service_name]] to [[account_name]] account. Amount: [[service_amount]]. Please review.',
    ctaText: 'View Billing',
    ctaLink: '[[portal_base_url]]',
    includeItemsTable: false,
    smsTemplate: '💰 [[tenant_name]]: [[user_name]] added [[service_name]] ([[service_amount]]) to [[account_name]]. Review: [[portal_base_url]]',
  },
  'task.created': {
    icon: 'clipboard_primary',
    accentColor: '#ea580c',
    buttonColor: '#111111',
    headerLine1: 'New task',
    headerLine2: 'incoming!',
    description: 'A new [[task_type]] task has been created and is due on [[task_due_date]]. Time to make things happen!',
    ctaText: 'View Task Details',
    ctaLink: '[[task_link]]',
    includeItemsTable: true,
    smsTemplate: '📋 [[tenant_name]]: New [[task_type]] task created, due [[task_due_date]]. View: [[portal_base_url]]',
  },
  'task.assigned': {
    icon: 'clipboard_primary',
    accentColor: '#ea580c',
    buttonColor: '#111111',
    headerLine1: 'You\'ve been',
    headerLine2: 'assigned!',
    description: 'You\'re up! A [[task_type]] task has been assigned to you, due [[task_due_date]]. You\'ve got this!',
    ctaText: 'Start Task',
    ctaLink: '[[task_link]]',
    includeItemsTable: true,
    smsTemplate: '👋 [[tenant_name]]: You\'re assigned to [[task_type]] task due [[task_due_date]]. Get started: [[portal_base_url]]',
  },
  'task.completed': {
    icon: 'check_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Task',
    headerLine2: 'complete!',
    description: 'Nice work! The [[task_type]] task has been completed by [[created_by_name]]. Another one done!',
    ctaText: 'View Results',
    ctaLink: '[[task_link]]',
    includeItemsTable: true,
    smsTemplate: '✅ [[tenant_name]]: [[task_type]] task completed by [[created_by_name]]. View: [[portal_base_url]]',
  },
  'task.overdue': {
    icon: 'clock_red',
    accentColor: '#dc2626',
    buttonColor: '#dc2626',
    headerLine1: 'Running',
    headerLine2: 'behind!',
    description: 'This [[task_type]] task was due on [[task_due_date]] and needs your attention. Let\'s get it wrapped up!',
    ctaText: 'Complete Task Now',
    ctaLink: '[[task_link]]',
    includeItemsTable: true,
    isUrgent: true,
    smsTemplate: '⏰ OVERDUE [[tenant_name]]: [[task_type]] task was due [[task_due_date]]. Action needed: [[portal_base_url]]',
  },
  'release.created': {
    icon: 'truck_orange',
    accentColor: '#FD5A2A',
    buttonColor: '#111111',
    headerLine1: 'Release',
    headerLine2: 'ready!',
    description: 'A [[release_type]] release has been created with [[items_count]] items. Amount due: [[amount_due]]. Ready when you are!',
    ctaText: 'View Release',
    ctaLink: '[[release_link]]',
    includeItemsTable: true,
    smsTemplate: '📤 [[tenant_name]]: [[release_type]] release created. [[items_count]] items, [[amount_due]] due. View: [[portal_base_url]]',
  },
  'release.approved': {
    icon: 'check_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Approved!',
    headerLine2: 'Let\'s roll.',
    description: 'Release [[release_number]] has been approved and is ready for processing. Full speed ahead!',
    ctaText: 'View Release',
    ctaLink: '[[release_link]]',
    includeItemsTable: true,
    smsTemplate: '✅ [[tenant_name]]: Release [[release_number]] approved! Ready for processing. View: [[portal_base_url]]',
  },
  'release.completed': {
    icon: 'check_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Out the',
    headerLine2: 'door!',
    description: 'Release [[release_number]] has been completed on [[release_completed_at]]. Thanks for your business!',
    ctaText: 'View Summary',
    ctaLink: '[[release_link]]',
    includeItemsTable: true,
    smsTemplate: '🎉 [[tenant_name]]: Release [[release_number]] completed! Thank you for your business. Details: [[portal_base_url]]',
  },
  'invoice.created': {
    icon: 'document_orange',
    accentColor: '#FD5A2A',
    buttonColor: '#FD5A2A',
    headerLine1: 'New',
    headerLine2: 'invoice!',
    description: 'Your invoice is ready! Amount due: [[amount_due]]. Click below to view and pay at your convenience.',
    ctaText: 'View & Pay Invoice',
    ctaLink: '[[portal_base_url]]',
    includeItemsTable: false,
    smsTemplate: '📄 [[tenant_name]]: New invoice ready - [[amount_due]] due. View & pay: [[portal_base_url]]',
  },
  'invoice.sent': {
    icon: 'document_orange',
    accentColor: '#ea580c',
    buttonColor: '#111111',
    headerLine1: 'Invoice',
    headerLine2: 'on its way!',
    description: 'Your invoice has been sent to [[account_contact_email]]. Keep an eye on your inbox!',
    ctaText: 'View Invoice',
    ctaLink: '[[portal_base_url]]',
    includeItemsTable: false,
    smsTemplate: '📧 [[tenant_name]]: Invoice sent to [[account_contact_email]]. View it: [[portal_base_url]]',
  },
  'payment.received': {
    icon: 'dollar_green',
    accentColor: '#16a34a',
    buttonColor: '#111111',
    headerLine1: 'Payment',
    headerLine2: 'received!',
    description: 'Thank you! We\'ve received your payment of [[amount_paid]]. You\'re all set!',
    ctaText: 'View Account',
    ctaLink: '[[portal_base_url]]',
    includeItemsTable: false,
    smsTemplate: '💚 [[tenant_name]]: Payment of [[amount_paid]] received. Thank you! View: [[portal_base_url]]',
  },
  'employee.invited': {
    icon: 'user_plus_primary',
    accentColor: '#ea580c',
    buttonColor: '#ea580c',
    headerLine1: 'You\'re',
    headerLine2: 'invited!',
    description: 'Welcome to the team! [[tenant_name]] has invited you to join their warehouse management system. Click below to set up your account and get started.',
    ctaText: 'Accept Invitation',
    ctaLink: '[[invite_link]]',
    includeItemsTable: false,
    smsTemplate: '🎉 [[tenant_name]]: You\'re invited! Set up your account: [[invite_link]]',
  },
  'custom': {
    icon: 'boxes_gray',
    accentColor: '#6b7280',
    buttonColor: '#111111',
    headerLine1: 'Heads',
    headerLine2: 'up!',
    description: 'This is a notification from [[tenant_name]]. Click below for more details.',
    ctaText: 'View Details',
    ctaLink: '[[portal_base_url]]',
    includeItemsTable: false,
    smsTemplate: '📢 [[tenant_name]]: You have a new notification. View details: [[portal_base_url]]',
  },
};

// Build the Cowboy-inspired email template HTML - Clean, Modern, Fun!
export function buildEmailTemplate(triggerEvent: string): string {
  const config = EMAIL_TEMPLATE_CONFIGS[triggerEvent] || EMAIL_TEMPLATE_CONFIGS['custom'];
  const iconDataUri = EMAIL_ICONS[config.icon];
  
  const urgentBanner = config.isUrgent ? `
              <!-- Urgent Banner -->
              <tr>
                <td style="background-color:${config.accentColor};color:#ffffff;padding:12px 24px;text-align:center;font-weight:600;font-size:14px;letter-spacing:0.5px;">
                  ⚠️ ACTION REQUIRED
                </td>
              </tr>` : '';

  return `<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${config.headerLine1} ${config.headerLine2}</title>
</head>
<body style="margin:0;padding:0;background-color:#f5f5f5;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;-webkit-font-smoothing:antialiased;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="min-height:100vh;background-color:#f5f5f5;">
    <tr>
      <td align="center" style="padding:40px 20px;">
        <table width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;width:100%;">
          
          <!-- Logo Header -->
          <tr>
            <td style="padding:24px 0;text-align:center;">
              <img src="[[brand_logo_url]]" alt="[[tenant_name]]" style="max-height:40px;max-width:180px;" onerror="this.style.display='none'">
            </td>
          </tr>
          
          <!-- Main Content Card -->
          <tr>
            <td>
              <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 6px -1px rgba(0,0,0,0.1),0 2px 4px -1px rgba(0,0,0,0.06);">
                ${urgentBanner}
                
                <!-- Content -->
                <tr>
                  <td style="padding:48px 40px;text-align:center;">
                    
                    <!-- Icon -->
                    <table width="100%" cellpadding="0" cellspacing="0" border="0">
                      <tr>
                        <td align="center" style="padding-bottom:24px;">
                          <img src="${iconDataUri}" width="72" height="72" alt="" style="display:block;">
                        </td>
                      </tr>
                    </table>
                    
                    <!-- Bold Split Headline -->
                    <h1 style="margin:0;font-size:48px;font-weight:700;color:#111111;line-height:1.1;letter-spacing:-0.5px;">
                      ${config.headerLine1}<br>${config.headerLine2}
                    </h1>
                    
                    <!-- Description -->
                    <p style="margin:24px auto 32px;font-size:17px;color:#6b7280;line-height:1.7;max-width:420px;">
                      ${config.description}
                    </p>
                    
                    <!-- CTA Button -->
                    <table cellpadding="0" cellspacing="0" border="0" style="margin:0 auto;">
                      <tr>
                        <td style="background-color:${config.buttonColor};border-radius:50px;">
                          <a href="${config.ctaLink}" target="_blank" style="display:inline-block;padding:16px 40px;color:#ffffff;text-decoration:none;font-weight:600;font-size:16px;letter-spacing:0.3px;">
                            ${config.ctaText}
                          </a>
                        </td>
                      </tr>
                    </table>
                    
                    <!-- Info Card -->
                    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin-top:40px;">
                      <tr>
                        <td style="background-color:#f9fafb;border-radius:12px;padding:24px;text-align:left;">
                          <table width="100%" cellpadding="0" cellspacing="0" border="0" style="font-size:14px;">
                            <tr>
                              <td style="padding:8px 0;color:#6b7280;width:120px;">Account:</td>
                              <td style="padding:8px 0;color:#111111;font-weight:600;">[[account_name]]</td>
                            </tr>
                            <tr>
                              <td style="padding:8px 0;color:#6b7280;">Date:</td>
                              <td style="padding:8px 0;color:#111111;font-weight:600;">[[created_at]]</td>
                            </tr>
                            <tr>
                              <td style="padding:8px 0;color:#6b7280;">Items:</td>
                              <td style="padding:8px 0;color:#111111;font-weight:600;">[[items_count]]</td>
                            </tr>
                          </table>
                        </td>
                      </tr>
                    </table>
                    
                    ${config.includeItemsTable ? `
                    <!-- Items Table -->
                    <div style="margin-top:32px;text-align:left;">
                      <p style="margin:0 0 16px;font-size:14px;font-weight:600;color:#111111;">Items included:</p>
                      [[items_table_html]]
                    </div>
                    ` : ''}
                    
                  </td>
                </tr>
              </table>
            </td>
          </tr>
          
          <!-- Divider -->
          <tr>
            <td style="padding:32px 0 0;">
              <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                  <td style="border-top:1px solid #e5e7eb;"></td>
                </tr>
              </table>
            </td>
          </tr>
          
          <!-- Footer -->
          <tr>
            <td style="padding:32px 20px 40px;text-align:center;">
              <!-- Company Logo in Footer -->
              <img src="[[brand_logo_url]]" alt="[[tenant_name]]" style="max-height:24px;max-width:120px;margin-bottom:16px;opacity:0.7;" onerror="this.style.display='none'">
              
              <p style="margin:0 0 12px;font-size:13px;color:#9ca3af;">
                Questions? Reach out at <a href="mailto:[[brand_support_email]]" style="color:#6b7280;text-decoration:underline;">[[brand_support_email]]</a>
              </p>
              <p style="margin:0;font-size:12px;color:#9ca3af;">
                © [[tenant_name]] • All rights reserved
              </p>
            </td>
          </tr>
          
        </table>
      </td>
    </tr>
  </table>
</body>
</html>`;
}

// Build SMS template - Fun, Friendly, and informative!
export function buildSmsTemplate(triggerEvent: string): string {
  const config = EMAIL_TEMPLATE_CONFIGS[triggerEvent] || EMAIL_TEMPLATE_CONFIGS['custom'];
  return config.smsTemplate;
}
